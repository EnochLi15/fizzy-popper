...
import { makeConfig, makeWebhookEvent, makeCard, makeGoldenTicket } from "./fixtures.js";
// Suppress log output
vi.mock("../src/log.js", () => ({
    header: vi.fn(),
    board: vi.fn(),
...
function getApp(config = makeConfig(), router = makeMockRouter(), supervisor = makeMockSupervisor()) {
    const server = new WebhookServer(config, router, supervisor);
    return { server, router: router, supervisor: supervisor };
}
// Helper to call the internal Hono app through WebhookServer
async function webhookRequest(server, body, headers = {}) {
...
async function statusRequest(server) {
    const app = server.app;
...
describe("WebhookServer", () => {
    describe("POST /webhook", () => {
        ...
        describe("dedup after dispatch", () => {
            it("does not dedup if dispatch throws", async () => {
                ...
                router.routeEvent.mockReturnValue({
                    type: "spawn",
                    card: makeCard(),
                    goldenTicket: makeGoldenTicket(),
                });
                supervisor.spawn.mockRejectedValueOnce(new Error("boom"));
                ...
                supervisor.spawn.mockResolvedValueOnce(undefined);
                const response2 = await webhookRequest(server, body);
    ...
    describe("GET /status", () => {
        ...
        it("returns active agents from supervisor", async () => {
            const supervisor = makeMockSupervisor();
            supervisor.getActiveRuns.mockReturnValue([
                {
                    card_number: 42,
                    card_title: "Fix bug",
