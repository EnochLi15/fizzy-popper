...
import { parseGoldenTicket, isGoldenTicket, verifyWebhookSignature, isWebhookFresh, FizzyClient, } from "../src/fizzy.js";
...
describe("FizzyClient", () => {
    ...
    describe("url construction", () => {
        it("constructs correct board list URL", async () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/123/boards", expect.objectContaining({
        ...
        it("strips trailing slash from api_url", () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/a/boards", expect.any(Object));
            fetchSpy.mockRestore();
        });
    });
    describe("listCards", () => {
        it("adds board_ids query params", async () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/123/cards?board_ids[]=b1&board_ids[]=b2", expect.any(Object));
            fetchSpy.mockRestore();
        });
    });
    ...
    describe("mutations", () => {
        it("closeCard sends POST to closure endpoint", async () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/123/cards/42/closure", expect.objectContaining({ method: "POST" }));
            fetchSpy.mockRestore();
        });
        it("triageCard sends column_id in body", async () => {
            const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(null, { status: 204 }));
            await client.triageCard(42, "col-done");
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/123/cards/42/triage", expect.objectContaining({
                method: "POST",
        ...
        it("postComment sends comment body", async () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/123/cards/42/comments", expect.objectContaining({
                method: "POST",
        ...
        it("toggleTag sends tag_title", async () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/123/cards/42/taggings", expect.objectContaining({
                method: "POST",
    ...
    describe("getIdentity", () => {
        it("fetches /my/identity without account slug", async () => {
            const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({ accounts: [] }), { status: 200 }));
            await client.getIdentity();
            expect(fetchSpy).toHaveBeenCalledWith("https://app.fizzy.do/my/identity", expect.any(Object));
            fetchSpy.mockRestore();
        });
    });
});
//# sourceMappingURL=fizzy.test.js.map